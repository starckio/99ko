...
function pageInstall(){
	...
	if(count($page->getItems()) < 1){
		...
		$pageItem->setContent('<p>L\'installation s\'est déroulée avec succès !<br>Rendez-vous sur le site officiel de 99ko pour télécharger des plugins et des thèmes.</p><p>Site officiel : <a href="http://99ko.org">http://99ko.org</a><br>Forum : <a href="http://99ko.org/support/">http://99ko.org/support/</a><br>Page Facebook : <a href="https://www.facebook.com/99kocms">https://www.facebook.com/99kocms</a></p>');
		$page->save($pageItem);
		$page = new page();
		$pageItem = new pageItem();
...
function pageEndAdminHead(){
    $core = core::getInstance();
	echo '<link href="'.$core->getConfigVal('siteUrl').'/plugin/page/other/admin.css" rel="stylesheet" type="text/css" />';
}

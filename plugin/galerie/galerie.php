...
function galerieEndFrontHead(){
    ...
    echo '<script type="text/javascript" src="'.$core->getConfigVal('siteUrl').'/plugin/galerie/other/public.js"></script>';
	echo '<link href="'.$core->getConfigVal('siteUrl').'/plugin/galerie/other/public.css" rel="stylesheet" type="text/css" />';
}

function galerieEndAdminHead(){
    $core = core::getInstance();
    echo '<script type="text/javascript" src="'.$core->getConfigVal('siteUrl').'/plugin/galerie/other/admin.js"></script>';
}
	...
	public function saveItem($obj){
		...
		if($id == ''){
			...
			if($upload == 'success'){
				...
				galerieResize(800, 800, UPLOAD.'galerie/', $obj->getId().'.jpg', UPLOAD.'galerie/', $obj->getId().'.jpg');
			}
			$this->items[] = $obj;
		}
		else{
